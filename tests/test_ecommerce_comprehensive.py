...
async def test_customer_crud_operations(ecommerce_db):
    ...
    await conn.execute(
        """INSERT INTO customers (_id, name, credit_limit, _deleted)
           VALUES ($1, $2, $3, $4)""",
        "CUST001", "Alice Johnson", 5000, False
    )
    
    # Update customer credit limit
    await conn.execute(
        "UPDATE customers SET credit_limit = $1 WHERE _id = $2",
        7500, "CUST001"
    )
    
    # Soft delete customer
    await conn.execute(
        "UPDATE customers SET _deleted = TRUE WHERE _id = $1",
        "CUST001"
    )
...
async def test_product_insert_and_update(ecommerce_db):
    ...
    await conn.execute(
        """INSERT INTO products (_id, name, description, price, _deleted)
           VALUES ($1, $2, $3, $4, $5)""",
        "PROD001", "Laptop", "High-performance laptop", Decimal("999.99"), False
    )
    
    # Update product price
    await conn.execute(
        "UPDATE products SET price = $1 WHERE _id = $2",
        Decimal("1099.99"), "PROD001"
    )
    
    # Update description
    await conn.execute(
        "UPDATE products SET description = $1 WHERE _id = $2",
        "Premium high-performance laptop", "PROD001"
    )
...
async def test_complete_order_workflow(ecommerce_db):
    ...
    await conn.execute(
        """INSERT INTO products (_id, name, description, price, _deleted)
           VALUES ($1, $2, $3, $4, $5)""",
        "PROD002", "Mouse", "Wireless mouse", Decimal("29.99"), False
    )
    await conn.execute(
        """INSERT INTO products (_id, name, description, price, _deleted)
           VALUES ($1, $2, $3, $4, $5)""",
        "PROD003", "Keyboard", "Mechanical keyboard", Decimal("89.99"), False
    )
    ...
    await conn.execute(
        """INSERT INTO orders (_id, cust_id, order_date, _deleted)
           VALUES ($1, $2, $3, $4)""",
        "ORD001", "CUST002", date(2026, 2, 12), False
    )
    ...
    await conn.execute(
        """INSERT INTO order_lines (_id, order_id, product_id, quantity, unit_price, _deleted)
           VALUES ($1, $2, $3, $4, $5, $6)""",
        "LINE001", "ORD001", "PROD002", 2, Decimal("29.99"), False
    )
    await conn.execute(
        """INSERT INTO order_lines (_id, order_id, product_id, quantity, unit_price, _deleted)
           VALUES ($1, $2, $3, $4, $5, $6)""",
        "LINE002", "ORD001", "PROD003", 1, Decimal("89.99"), False
    )
...
async def test_hard_delete_operations(ecommerce_db):
    ...
    await conn.execute(
        """INSERT INTO customers (_id, name, credit_limit, _deleted)
           VALUES ($1, $2, $3, $4)""",
        "CUST003", "Charlie Brown", 3000, False
    )
...
async def test_order_cancellation_cascade(ecommerce_db):
    ...
    await conn.execute(
        """INSERT INTO customers (_id, name, credit_limit, _deleted)
           VALUES ($1, $2, $3, $4)""",
        "CUST020", "Diana Prince", 15000, False
    )
    
    await conn.execute(
        """INSERT INTO products (_id, name, description, price, _deleted)
           VALUES ($1, $2, $3, $4, $5)""",
        "PROD020", "Monitor", "4K Monitor", Decimal("399.99"), False
    )
    
    await conn.execute(
        """INSERT INTO orders (_id, cust_id, order_date, _deleted)
           VALUES ($1, $2, $3, $4)""",
        "ORD020", "CUST020", date(2026, 2, 12), False
    )
    
    await conn.execute(
        """INSERT INTO order_lines (_id, order_id, product_id, quantity, unit_price, _deleted)
           VALUES ($1, $2, $3, $4, $5, $6)""",
        "LINE020", "ORD020", "PROD020", 2, Decimal("399.99"), False
    )
...
async def test_mixed_operations_single_transaction(ecommerce_db):
    ...
    async with conn.transaction():
        await conn.execute(
            """INSERT INTO customers (_id, name, credit_limit, _deleted)
               VALUES ($1, $2, $3, $4)""",
            "CUST030", "Eve Anderson", 8000, False
        )
        
        await conn.execute(
            """INSERT INTO products (_id, name, description, price, _deleted)
               VALUES ($1, $2, $3, $4, $5)""",
            "PROD030", "Tablet", "10-inch tablet", Decimal("299.99"), False
        )
...
async def test_null_values_in_description(ecommerce_db):
    ...
    await conn.execute(
        """INSERT INTO products (_id, name, description, price, _deleted)
           VALUES ($1, $2, $3, $4, $5)""",
        "PROD040", "Generic Product", None, Decimal("49.99"), False
    )
    
    # Update to set description
    await conn.execute(
        "UPDATE products SET description = $1 WHERE _id = $2",
        "Now has a description", "PROD040"
    )
    
    # Update to set back to NULL
    await conn.execute(
        "UPDATE products SET description = NULL WHERE _id = $1",
        "PROD040"
    )
...
async def test_decimal_precision(ecommerce_db):
    ...
    for prod_id, price in test_prices:
        await conn.execute(
            """INSERT INTO products (_id, name, description, price, _deleted)
               VALUES ($1, $2, $3, $4, $5)""",
            prod_id, f"Product {prod_id}", "Test product", price, False
        )
    ...
    received_prices = {msg.after["_id"]: float(msg.after["price"]) 
                      for msg in messages}

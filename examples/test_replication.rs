...
async fn setup_database() -> anyhow::Result<(String, u16)> {
    ...
    // Connect and setup
    let dsn = format!("host=localhost port={} user=test password=test dbname=testdb", port);
    ...
    client.execute("CREATE TABLE test (id INT PRIMARY KEY, val TEXT)", &[]).await?;
    client.execute("ALTER TABLE test REPLICA IDENTITY FULL", &[]).await?;
    
    println!("Creating publication...");
    client.execute("CREATE PUBLICATION test_pub FOR ALL TABLES", &[]).await?;
    ...
    println!("\nInserting test data...");
    client.execute("INSERT INTO test VALUES (1, 'hello')", &[]).await?;
    client.execute("INSERT INTO test VALUES (2, 'world')", &[]).await?;
            ...
            Ok(Ok(Some(event))) => {
                match event {
                    ReplicationEvent::XLogData { wal_start, wal_end, data, .. } => {
                        println!("📦 XLogData: wal_start={} wal_end={} bytes={}", 
                                 wal_start, wal_end, data.len());
                        message_count += 1;
                    ...
                    ReplicationEvent::KeepAlive { wal_end, reply_requested, .. } => {
                        println!("💓 KeepAlive: wal_end={} reply_requested={}", 
